...
var subcolorizeToCallback = (key, diff, output, color, indent) => {
  let subvalue;
      ...
      if (looksLikeDiff) {
        ...
        for ([op, subvalue] of diff) {
          if (op === " " && subvalue == null) {
            ...
            if (![" ", "~", "+", "-"].includes(op)) {
              throw new Error(
                `Unexpected op '${op}' in ${JSON.stringify(diff, null, 2)}`
              );
            }
            if (op === "~") {
              op = " ";
...
export const colorizeToCallback = (diff, output) =>
  subcolorizeToCallback("", diff, output, " ", "");

export const colorizeToArray = function(diff) {
  const output: string[] = [];
...
export const colorize = function(diff, options) {
  ...
  colorizeToCallback(diff, function(color, line) {
    if (options.color != null ? options.color : true) {
      return output.push(
        ((options.theme != null ? options.theme[color] : undefined) != null
          ? options.theme != null
            ? options.theme[color]
            : undefined
